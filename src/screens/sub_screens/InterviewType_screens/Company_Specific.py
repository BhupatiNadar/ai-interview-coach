...
from src.agents.Company_Specific_Agent import generate_questions, evaluate_answers


def company_specific():

    st.header("🏢 Company Specific Interview")
    st.caption("Tailored questions based on company values, required skills, and interview patterns")
    ...
    with col1:
        # Pre-generation form for company and role
        if "questions" not in st.session_state or st.session_state["questions"] == None:
            
            with st.container(border=True):
                st.subheader("🎯 Interview Target")
                company_name = st.text_input("Target Company", placeholder="e.g. Google, Amazon, OpenAI, Netflix...")
                role_name = st.text_input("Target Role", placeholder="e.g. Senior Software Engineer, Product Manager...")
                
                if st.button("Generate Tailored Questions", width='stretch'):
                    if not company_name or not role_name:
                        st.warning("Please enter both Company and Role to generate questions.")
                    else:
                        with st.spinner(f"🤖 Generating questions tailored for {role_name} at {company_name}..."):
                            result = generate_questions(resume, company_name, role_name)
        ...
        company = st.session_state.get("target_company", "the company")

        if current >= len(questions):
            st.success(f"🎉 {company} Interview Completed!")
            ...
            if not st.session_state.get("interview_saved", False):
                with st.spinner("🤖 Evaluating your answers..."):
                    ...
                    interview_response = save_interview_result(
                        ...
                        interview_type=f"Company Specific - {company}",
        ...
        st.info(f"💡 **Tip:** Keep {company}'s core values in mind while answering.")

        answer = st.text_area("Your Answer", height=200, key=f"answer_{current}", placeholder="Type your detailed answer here...")
