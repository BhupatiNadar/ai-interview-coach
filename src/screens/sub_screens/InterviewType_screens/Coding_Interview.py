...
from src.agents.Coding_Interview_Agent import generate_questions, evaluate_answers


def coding_interview():

    st.header("🧑‍💻 Coding Interview")
    st.caption("Data Structures & Algorithms Problem Solving")
    ...
    with col1:
        ...
        if st.button("Generate Coding Problems", width='stretch'):
            with st.spinner("🤖 Analyzing resume and generating coding problems..."):
        ...
        if "questions" not in st.session_state or st.session_state["questions"] == None:
            st.info("Click the button above to generate 5 coding challenges tailored to your skill level.")
            return
        ...
        if current >= len(questions):
            ...
            if not st.session_state.get("interview_saved", False):
                with st.spinner("🤖 Evaluating your code for correctness and complexity..."):
                    interview_answers = []
                    ...
                    try:
                        ...
                        st.success("✅ Code evaluated and saved successfully!")
        ...
        with st.container(border=True):
            st.subheader(f"Problem {current + 1} of {len(questions)}")
            st.write(f"**{question}**")

        st.info("💡 **Tip:** Write clear, optimized code and include comments explaining your time and space complexity.")
        ...
        with col_sub:
            if st.button("Submit Code", key="submit_answer",width='stretch'):
                if answer.strip() == "":
